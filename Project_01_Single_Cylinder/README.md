...
| **X2** | Input | Home Sensor | A접점 | 실린더 후진 완료 감지 오토스위치 (A_HOME_LS) |
| **X3** | Input | Forward Sensor | A접점 | 실린더 전진 완료 감지 오토스위치 (A_FWD_LS) |
...
stateDiagram-v2
    [*] --> M0_IDLE
    M0_IDLE --> M1_FORWARD : X0(Start) AND X2(후진완료)
    M1_FORWARD --> M2_DWELL : X3(전진완료)
    M1_FORWARD --> M0_IDLE : X1(E-Stop) OR M100(오류)
    M2_DWELL --> M3_RETURN : T0(1초 타이머 완료)
    M2_DWELL --> M0_IDLE : X1(E-Stop)
    M3_RETURN --> M0_IDLE : X2(후진완료) OR X1(E-Stop)